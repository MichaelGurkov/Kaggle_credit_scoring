...
preprocess_recipe = recipe(formula = target ~ ., data = train_set) %>%
  ...
  step_mutate_at(c("income", "debt_ratio"),
...
# alternative_recipe = recipe(formula = target ~ ., data = train_set) %>%
#   step_mutate(target = factor(target, levels = c(1,0))) %>% 
...
#                  fn = ~ DescTools::Winsorize(., probs = c(0,0.95),
#                                              na.rm = TRUE)) %>% 
#   step_YeoJohnson(c("monthly_income", "debt_ratio"))
...
roc_auc_df = roc_auc_df %>% 
  mutate(roc_auc = map_dbl(temp_var, function(temp_name){
    
    temp_estimate = workflow() %>% 
      add_formula(formula(glue("target ~ ", temp_name))) %>% 
      add_model(logistic_reg()) %>% 
      fit(train_set_transformed) %>% 
      predict(type = "prob",train_set_transformed) %>% 
      select(estimate = .pred_1) %>% 
      bind_cols(select(train_set_transformed, truth = target)) %>% 
...
roc_auc_df %>% 
  # mutate(roc_auc = if_else(roc_auc < 0.5, 1 - roc_auc, roc_auc)) %>% 
  mutate(roc_auc = roc_auc - 0.5) %>% 
  ggplot(aes(x = reorder(temp_var, roc_auc), y = roc_auc)) + 
  geom_col() + 
  coord_flip() + 
